...
def main():
    ...
    try:
        # Select and process a single task
        task_file = "passed_agent_passes.json"  # Update this path as needed
        selected_task = select_task(task_file)
        
        print(f"\nProcessing task: {selected_task.get('task_id', 'Unknown Task')}")
        result = generator.process_task(selected_task)
        
        # Save results with task ID in filename
        task_id = selected_task.get('task_id', 'unknown_task')
