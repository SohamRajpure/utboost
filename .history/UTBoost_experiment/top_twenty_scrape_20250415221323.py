...
for agent in TOP_AGENTS:
    ...
    with open(all_preds_path, 'r') as f:
        for i, line in enumerate(f):
            if i >= 5:  # Only process first 5 tasks
                break
                
            try:
                data = json.loads(line)  # Parse the line as-is without any modification
                ...
                print(f"Error parsing JSON for {agent} at line {i+1}: {str(e)}")
                continue
