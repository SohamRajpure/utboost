...
for agent in TOP_AGENTS:
    ...
    # Process the first 5 tasks from all_preds.jsonl
    task_count = 0
    with open(all_preds_path, 'r') as f:
        for line in f:
            if task_count >= 5:  # Only process first 5 tasks
                break
            task_count += 1  # Increment counter for each task processed
            try:
                ...
                # Only create task directory and process if this is one of the first 5 tasks
                ...
                print(f"Error parsing JSON for {agent} at line {task_count+1}: {str(e)}")
                continue
