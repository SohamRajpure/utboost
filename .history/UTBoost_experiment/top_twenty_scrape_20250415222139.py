...
evaluation_dir = "../evaluation/verified"  # Path relative to UTBoost_experiment directory

# Create a directory to store task-specific information if it doesn't exist
tasks_dir = "tasks"
os.makedirs(tasks_dir, exist_ok=True)

# Keep track of processed tasks across all agents
processed_tasks = set()
MAX_TASKS = 5
...
for agent in TOP_AGENTS:
    ...
    if len(processed_tasks) >= MAX_TASKS:
        break
        
    # Paths to the agent's files
    all_preds_path = os.path.join(evaluation_dir, agent, "all_preds.jsonl")  # Changed to .jsonl
    ...
    # Process tasks from all_preds.jsonl
    with open(all_preds_path, 'r') as f:
        for line in f:
            ...
            try:
                ...
                if instance_id in processed_tasks:
                    continue
                ...
                processed_tasks.add(instance_id)
                ...
                print(f"Error parsing JSON for {agent} at line {len(processed_tasks)+1}: {str(e)}")
                continue

print(f"Processed {len(processed_tasks)} tasks in total")
