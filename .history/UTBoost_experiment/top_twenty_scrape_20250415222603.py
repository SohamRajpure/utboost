...
for agent in TOP_AGENTS:
    ...
    # Process the first 5 lines from all_preds.jsonl
    line_count = 0
    with open(all_preds_path, 'r') as f:
        for line in f:
            if line_count >= 5:  # Only process first 5 lines
                break
            line_count += 1  # Increment after processing each line
            try:
                ...
                print(f"Error parsing JSON for {agent} at line {line_count+1}: {str(e)}")
                continue
